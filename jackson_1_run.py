"""
  ...
  These transition probabilities are as follows:
    Node     m_i  p_i0   p_i1  p_i2    p_i3    p_i4
     i                                        (leave)
     0       1.0   0      0    0.4     0.4     0.2
     1       2.0   0      0    0.4     0.4     0.2 
     2       1.0   0.5    0.5    0       0      0
     3       1.0   0.5    0.5    0       0      0
...
class Msg(Process):
    ...
    def trace(self,message):
        if MTRACING: print("{0:7.4f} {1:3s} {2:10s}".format(self.sim.now(),self.name, message))
...
GTRACING = True  ## tracing Messages Source?

startNode = 0     ## Messages always enter at node 0
ran.seed(77777)
MTRACING = True ## tracing Message action?

responseTimeReq = 100

mean=[1.0, 2.0, 1.0, 1.0]       ## service times, seconds
P = [[0,   0,   0.4, 0.4, 0.2], ## transition matrix P_ij
     [0,   0,   0.4, 0.4, 0.2],
     [0.5, 0.5,   0,   0,   0],
     [0.5, 0.5,   0,   0,   0]]
...
class JacksonnetworkModel(Simulation):
    def run(self):
        ...
        self.node = [Resource(1,sim=self),Resource(1,sim=self),Resource(1,sim=self), Resource(1,sim=self)]
