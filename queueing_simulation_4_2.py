...
class Job(Process):
    ...
    def visit(self, processors):
        ...
        if self.i%2 == 0:
            ...
            yield request,self,processors[3]
            wait = float(now()-arrive)
            print ("%8.5f %s: Waited for %6.5f for processor %i"%(now(),self.name,wait, 3))
            tiw = expovariate(1.0/serviceTime)
            yield hold,self,tiw                      
            yield release,self,processors[3]
            
        j = random.randint(1, 10)
        wait = now()
        while j < 4:
            if j <= 2:
                yield request,self,processors[4]
                wait = float(now()-wait)
                print ("%8.5f %s: Waited for %6.5f for processor %i"%(now(),self.name,wait, 4))
                tiw = expovariate(1.0/serviceTime)
                yield hold,self,tiw                      
                yield release,self,processors[4]
            elif j > 2 and j <= 4:
                yield request,self,processors[5]
                wait = float(now()-wait)
                print ("%8.5f %s: Waited for %6.5f for processor %i"%(now(),self.name,wait, 5))
                tiw = expovariate(1.0/serviceTime)
                yield hold,self,tiw                      
                yield release,self,processors[5]
...
processors = [Resource(name="VM1"), Resource(name="VM2"), Resource(name="VM3"), Resource(name="VM4"), Resource(name="VM5"), Resource(name="VM6")]
